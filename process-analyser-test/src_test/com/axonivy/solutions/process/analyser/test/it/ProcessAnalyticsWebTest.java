...
public class ProcessAnalyticsWebTest extends WebBaseSetup {
  ...
  private void verifyAndSelectAProcess(String startElementName) {
    ...
    var selectionPanel =
        $(PROCESS_DROPDOWN_CSS_SELECTOR + CASCADE_DROPDOWN_LIST_SUFFIX).shouldBe(visible, DEFAULT_DURATION);
    ...
    SelenideElement targetElement =
        selectionPanel.$$(" li").stream().filter(item -> processFileAndStartName.equals(item.text())).findAny()
