...
public class ProcessesMonitorUtils {
  ...
  private static void handleFrequencyForAlternativePath(Alternative alternative, List<Node> results, List<ICase> cases) {
  ...
  private static void updateFrequencyForCaseWithSimpleAlternative(List<AlternativePath> paths, List<Node> results,
    ...
    cases.stream().forEach(currentCase -> {
      List<Long> taskStartsInCase = currentCase.tasks().all().stream().map(iTask -> iTask.getStartSwitchEvent().getId()).toList();
      Ivy.log().warn(taskStartsInCase);
//      AlternativePath runningPath = paths.stream().filter(path-> taskStartsInCase)
  ...
  private static void followPath(AlternativePath path, SequenceFlow currentFlow) {
    ...
    if(ProcessUtils.isTaskSwitchEvent(destinationElement)) {
      Ivy.log().warn(destinationElement.getPid());
//      path.setTaskSwitchEventOnPath(destinationElement.getPid().toString());
    }
